...
static void m3u2pls(char *ipath)
{
	...
	do
	{
		...
		if (ifp == NULL)
		{
			printf("File '%s' cannot open.¥n", ipath);
			break;
		}
		...
		if (!overwrite)
		{
			...
			if (ofp != NULL)
			{
				...
				fclose(ofp);	ofp = NULL;
				do {
					printf("File '%s' already exists.¥n", opath);
		...
		if (ofp == NULL)
		{
			printf("File '%s' cannot open(w).¥n", opath);
			break;
		}
		while (!feof(ifp))
		{
	...
	if (num)
	{
		fprintf(ofp, "NumberOfEntries=%d¥n", num);
		fprintf(ofp, "Version=%d¥n", 2);
	}
